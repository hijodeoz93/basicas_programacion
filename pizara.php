...
<body>
        <?php
            while($colTareas=mysqli_fetch_assoc($tareas)or die($con->error)){
                echo "$colTareas[Tipo_tarea]</br>$colTareas[Nombre_archivo]</br>";    
            }
        ?>
        <?
            ...
            $colTareasI=mysqli_fetch_assoc($tareasI)or die($con->error);
            print_r($colTareasI);
            while($ColtareasI){
                
               echo "$colTareasI[Tipo_tarea]<input type='file' name='archivo'>";
            }
